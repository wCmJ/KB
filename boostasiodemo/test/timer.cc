...
int main()
{
  try
  {
    ...
    std::cout << "Starting synchronous wait";print_time();
    timer.wait();
    std::cout << "Finished synchronous wait";print_time();

    timer.expires_after(asio::chrono::seconds(5));
    std::cout << "Starting asynchronous wait";print_time();
    timer.async_wait(&handle_timeout);
    io_context.run();
    std::cout << "Finished asynchronous wait";print_time();
  }
